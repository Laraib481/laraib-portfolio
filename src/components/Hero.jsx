...
export default function Hero() {
  ...
  // 1. First View Fade-Out + Scale Up & Blur
  const view1Opacity = useTransform(scrollYProgress, [0, 0.3], [1, 0]);
  const view1Y = useTransform(scrollYProgress, [0, 0.3], [0, -60]);
  const view1Scale = useTransform(scrollYProgress, [0, 0.3], [1, 1.15]); // Enlarges title
  const view1Blur = useTransform(scrollYProgress, [0, 0.3], ['blur(0px)', 'blur(12px)']); // Blurs title

  // 2. Second View Clear Sharp Fade-In
  const view2Opacity = useTransform(scrollYProgress, [0.35, 0.65], [0, 1]);
  const view2Y = useTransform(scrollYProgress, [0.35, 0.65], [30, 0]);

  // 3. Central Image Morphing & 3D Flip Transformation
    ...
    <div ref={containerRef} className="relative h-[300vh] w-full">
      <div className="sticky top-4 sm:top-6 mx-auto max-w-6xl px-3 sm:px-4">
        <div className="bg-[#F3F2EE] rounded-[28px] sm:rounded-[40px] h-[92vh] sm:h-[88vh] min-h-[620px] p-4 sm:p-10 md:p-12 relative overflow-hidden shadow-2xl border border-neutral-300/80 flex flex-col justify-between">
          ...
          >
            {/* <div className="w-full flex justify-between items-center text-[10px] sm:text-xs md:text-sm font-mono font-bold tracking-wider text-black mb-4 sm:mb-10">
              <span className="flex items-center gap-1">
                <Sparkles size={14} className="fill-black sm:w-4 sm:h-4" /> ©2026
              </span>
              <span id="about" className="flex items-center gap-1 text-right pointer-events-auto">
                /CREATING SINCE 2020 <Zap size={14} className="fill-black sm:w-4 sm:h-4" />
              </span>
            </div> */}

            <h1   id='about'  className="text-4xl xs:text-5xl sm:text-7xl md:text-8xl lg:text-[104px] font-black tracking-tighter text-black uppercase text-center leading-[0.9] sm:leading-[0.88] mt-2 sm:mt-8 select-none">
            ...
            <div className="md:col-span-4 space-y-2 sm:space-y-4 text-xs sm:text-sm font-medium text-neutral-700 leading-relaxed bg-white/40 md:bg-transparent p-3 sm:p-0 rounded-xl backdrop-blur-xs md:backdrop-blur-none border border-black/5 md:border-none">
              ...
              <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 pt-1">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 sm:px-5 sm:py-2.5 bg-[#0077B5] hover:bg-[#005E93] text-white rounded-full text-xs font-bold transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 sm:px-5 sm:py-2.5 bg-black hover:bg-neutral-800 text-white rounded-full text-xs font-bold transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                >
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
            ...
            >
              ...
              <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)]">
                <img
                  src={myFrontImage}
                  alt="Back View"
                  className="w-full h-full object-cover saturate-125 brightness-95"
                />
              </div>
            </motion.div>
          </div>
